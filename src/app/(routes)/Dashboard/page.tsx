...
function Dashboard() {
  ...
  const getAllExpenses = async() => {
    ...
    }).from(Budgets).leftJoin(Expenses,eq(Budgets.id,Expenses.budgetId)).where(eq(Budgets.createdBy,user?.primaryEmailAddress?.emailAddress)).orderBy(desc(Expenses.id))
    setExpensesList(result) ;
  }
  return (
    <div className='p-8'>
     <h2 className='font-bold text-3xl'> Hi, {user?.fullName} 🤝</h2>
     <p className='text-gray-500'>Here's what's happening with your money. Let us manage your expenses!</p>
